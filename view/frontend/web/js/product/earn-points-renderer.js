// copy of vendor/mirasvit/module-rewards/src/RewardsCatalog/view/frontend/web/js/product/earn-points-renderer.js
define([
    'jquery',
    'priceUtils',
    'priceBox'
    ...
    $.widget('mage.earnPointsRenderer', {
        component: 'Mirasvit_RewardsCatalog/js/product/earn-points-renderer', // Breeze change
        options: {},
        pointsXhr: [],
        productData: {},
        priceBoxEl: null,
        ...
        _init: function() {
            ...
            $('.input-text.qty', self.productMainBlock).on('input', function() { // Breeze change: keyup() => on('input')
            ...
            requestData[requestUuid].pointsXhr[this.productId] = $.ajax({
                ...
                success: function (data) {
                    for (var k in data) {
                        ...
                        if (data[k].points > 0) {
                            element.find('.price').html(data[k].label); // Breeze change: $('.price', element) => element.find('.price')
                        } else {
                            element.find('.price').html(''); // Breeze change: $('.price', element) => element.find('.price')
                        }
                        element.find('.points-loader').hide(); // Breeze change: $('.points-loader', element) => element.find('.points-loader')
                    }
                },
                complete: function() {
                    activeRequests = 0;
