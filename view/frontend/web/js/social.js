...
function fbShare() {
    $.ajax({ // Breeze change: jQuery => $
        url: window.fbShareUrl + '?url=' + rewardsCurrentUrl,
        type: 'POST',
        dataType: 'JSON',
        complete: function (data) {
            $('#status-message').html(data.responseText); // Breeze change: jQuery => $
            $('#facebook-share-message').html(''); // Breeze change: jQuery => $
        }
    });
}
...
function fbLike() {
    $.ajax({ // Breeze change: jQuery => $
        ...
        complete: function (data) {
            $('#status-message').html(data.responseText); // Breeze change: jQuery => $
            $('#facebook-message').html(''); // Breeze change: jQuery => $
        }
    });
}
...
function tweet() {
    $.ajax({ // Breeze change: jQuery => $
        ...
        complete: function (data) {
            $('#status-message').html(data.responseText); // Breeze change: jQuery => $
            $('#twitter-message').html(''); // Breeze change: jQuery => $
        }
    });
}
...
function pinIt() {
    $.ajax({ // Breeze change: jQuery => $
        ...
        complete: function (data) {
            $('#status-message').html(data.responseText); // Breeze change: jQuery => $
            $('#pinterest-message').html(''); // Breeze change: jQuery => $
        }
    });
}
